...
export async function batchFetchOgImages(
  ...
  for (let i = 0; i < missing.length; i += CONCURRENCY) {
    ...
    // Each batch fires `fetchOgImage` in parallel and writes the result
    // straight into the shared `results` map. We don't care about per-call
    // success/failure here — failures are silently dropped because
    // `fetchOgImage` already swallows them.
